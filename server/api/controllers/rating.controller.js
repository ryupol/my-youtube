...
const updateLikeByID = async (req, res) => {
  ...
  try {
    ...
    if (!actionExists) {
      ...
      updateRating = await Rating.updateOne({ user_id, video_id }, { rating: "like" });
      incLikes = 1;
      incDislikes = -1;
    }

    await Videos.updateOne({ _id: video_id }, { $inc: { likes: incLikes, dislikes: incDislikes } });
...
const updateDislikeByID = async (req, res) => {
  ...
  try {
    ...
    if (!actionExists) {
      updateRating = await Rating.create({ video_id, user_id, rating: "dislike" });
      incDislikes = 1;
    } else if (action.rating === "dislike") {
      updateRating = await Rating.findOneAndDelete({ user_id, video_id });
      incDislikes = -1;
    } else if (action.rating === "like") {
      updateRating = await Rating.updateOne({ user_id, video_id }, { rating: "dislike" });
      incLikes = -1;
      incDislikes = 1;
    }
...
export { getAllRating, updateLikeByID, updateDislikeByID, getRatingByVideoID };
